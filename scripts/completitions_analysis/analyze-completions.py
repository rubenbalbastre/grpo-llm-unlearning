...
def download_wandb_completion_files(
    *,
    ...
    for run in api.runs(wandb_path):
        ...
        if job_type and str(job_type) != TRAINING_JOB_TYPE:
            continue
...
def sum_source_rows(summaries: SummaryTables, field_name: str) -> int:
    return sum(getattr(stats, field_name) for stats in summaries.source.values())


def print_report(args: argparse.Namespace, files: list[DownloadedCompletion], result: AnalysisResult) -> None:
    target_entities = REFUSAL_PATTERNS[args.concept]
    near_target_rows = sum_source_rows(result.summaries, "near_target_rows")
    near_without_exact_rows = sum_source_rows(result.summaries, "near_without_exact_rows")
    exact_entity_rows = sum_source_rows(result.summaries, "exact_entity_rows")
    ...
    print(f"rows with fuzzy reward match: {near_target_rows}")
    print(f"rows with fuzzy reward match but no exact target-entity match: {near_without_exact_rows}")
    print(f"rows with exact reward-list entity match: {exact_entity_rows}")

    print("\nby source kind:")
    for source_kind, stats in sorted(result.summaries.source.items()):
        print_summary_line(f"  {source_kind}", stats)

    print("\nby training mode:")
    for (training_mode, source_kind), stats in sorted(result.summaries.mode.items()):
        print_summary_line(f"  {training_mode}/{source_kind}", stats)

    if result.entity_counts:
        print("\ntop exact reward-list entities:")
        for entity, count in result.entity_counts.most_common(20):
            print(f"  {entity}: {count}")

    print_examples(result.findings, max_examples=args.max_examples)


def print_summary_line(label: str, stats: SummaryRow) -> None:
    print(
        f"{label}: rows={stats.matching_filter_rows} "
        f"near_target={stats.near_target_rows} "
        f"near_without_exact={stats.near_without_exact_rows} "
        f"exact_entity={stats.exact_entity_rows}"
    )


def print_examples(findings: list[Finding], max_examples: int) -> None:
    print("\nexamples:")
    for finding in findings[:max_examples]:
        ...
        print(
            ...
            f"fuzzy_reward={finding.fuzzy_reward:.3f} "
            f"exact_target={finding.exact_target} "
...
def main() -> None:
    ...
    if not project:
        raise SystemExit("WANDB_PROJECT is required in the repository .env file or environment.")
    ...
    if not files:
        raise SystemExit(f"No W&B completion tables found for concept {args.concept!r}.")
    result = scan_completion_files(
        files,
        concept=args.concept,
    )
    print_report(args, files, result)
