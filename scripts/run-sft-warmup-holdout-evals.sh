...
submit_warmup_holdout() {
  ...
  sbatch --parsable \
    ...
    --time="01:30:00" \
    --partition="hopper" \
    --wrap="cd ${REPO_DIR} && if [ -f '${run_dir}/${LOW_REWARD_STOP_MARKER}' ]; then echo 'Skipping hold-out because stop marker exists: ${run_dir}/${LOW_REWARD_STOP_MARKER}'; cat '${run_dir}/${LOW_REWARD_STOP_MARKER}'; exit 0; fi && source \$REPO_DIR/anaconda3_bis/etc/profile.d/conda.sh && conda activate py312_cu118_bis && python eval/hold_out_styles/generate-and-analyze-completions.py concept='${target}' model_name_or_path='${final_model}' paths.storage_root='${STORAGE_ROOT}'"
}
